---
title: "Windows, averages, and similarity — cheap statistics for an agent's instincts"
sidebar:
  order: 7
---
...
The common philosophy first: these statistics were chosen because they are **cheap, local, and understandable**. They run on every call, hold a bounded amount of memory, and a human can recompute them by hand. Heavier machinery exists (embeddings, semantic models — see [memory](/extensions/memory/) for where you'd plug those in), but for *is this behavior repeating?* the cheap tools are the right tools — and their weaknesses are known and fenced off rather than hidden.
...
This is the loop detector's memory ([loop detection](/safety/loop-detection/)): the last **50** operations (100 when wired through the `DetectionManager`). Repetition is counted *within* the window — so a pattern must be both *repeated* and *recent* to trigger.
...
loopctl's tool health scoring ([tool health](/safety/tool-health/)) tracks success with a 70/30 blend:
...
This is the **Jaccard similarity** of the two word sets, and it powers [convergence detection](/safety/convergence/) — noticing a model giving near-identical *final answers*, turn after turn.
...
- [Loop detection](/safety/loop-detection/) · [convergence](/safety/convergence/) · [tool health](/safety/tool-health/) — the systems these statistics live in.
- [Memory](/extensions/memory/) — the one place loopctl does heavier scoring, with its full formula.
- [Soft and hard errors](/principles/soft-and-hard-errors/) — what happens *after* a statistic crosses its line.
