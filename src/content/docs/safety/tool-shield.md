---
title: "The tool safety shield — scoring dangerous commands [feature: tool_shield]"
sidebar:
  order: 7
---
...
- **Tool health** ([previous page](/safety/tool-health/)): a shield refusal is an `is_error` result, so repeated blocks count as failures toward the tool's breaker. Once that opens, the engine's gate refuses first — the model sees "temporarily unavailable" rather than the shield's reason. That precedence is pinned; plan your messaging accordingly.
...
- [Middleware](/safety/middleware/) — the pipeline the shield lives in.
- [Tool health](/safety/tool-health/) — the breaker its refusals feed.
