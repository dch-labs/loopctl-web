---
title: "`src/tool/builtin.rs` + `src/tool/builtin/think.rs` — the built-in tools"
---
...
- Registration is the only entry: `registry.register(ThinkTool)` — see [tools](/core-data/tools/).
- No state, no filesystem, no network — safe in any pipeline position and under parallel dispatch.

Deep dive: [Built-in tools](/core-data/tools/#built-in-tools--thinktool) · [File reference: tool.rs](/file-reference/tool/).
