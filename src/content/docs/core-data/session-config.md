---
title: "Configuration — SessionConfig, RunConfig, and who owns what"
sidebar:
  order: 6
---
...
| `parallel_tool_dispatch` | `ParallelDispatchConfig` | sequential, max 8 | How tool calls within a turn run. See [tool dispatch](/engine/tool-dispatch/). |
...
| `memory_top_k` | `usize` | `3` | How many memory entries to retrieve each turn. `0` disables memory injection. See [memory](/extensions/memory/). |
...
That split is exactly why a turn limit can change between two `run()` calls while the window policy stays stable for the session's life — and why the brain serializes with none of these numbers inside it: they are re-supplied, fresh, on every single step (see [the state machine](/engine/state-machine/)).
...
1. **Cargo features** decide what is compiled at all (`streaming`, `hooks`, `tool_health`, `tool_shield`, `redaction`, providers, `mcp`, `derive`, `testing`, ...). The crate's default is **no features** — a minimal build has no provider clients, no hooks, nothing optional. See the [provider overview](/providers/overview/) for the full list.
2. **`RequestOptions`** (set with `agent.set_request_options(...)`) attach to every model request: a per-request model override, a forced JSON response format, or a strict-tools constraint. See [structured output](/integration/structured-output/).

And two moments when config *changes* mid-life are legitimate:

- [`switch_model`](/engine/model-switch/) can update the context window when the model changes.
...
Details: [presets](/integration/presets/).

---

## Related pages

- [Compaction](/engine/compaction/) — what the window/threshold numbers actually do.
- [The driver](/engine/driver-loop/) — how config flows into the brain as `MachinePolicy`.
- [File reference: config.rs](/file-reference/config/)
