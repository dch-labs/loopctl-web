---
title: "Stream events — how a reply arrives piece by piece"
sidebar:
  order: 4
---
...
- **Rate-limit escalation ≠ same-model fallback.** By default, exhausting rate-limit retries escalates to the model-level [fallback manager](/safety/fallback/) rather than hammering the same endpoint unstreamed.
...
The engine forwards every accepted event to [observers](/extensions/observers/): `on_text_delta` per text fragment, `on_thinking_delta` per reasoning fragment, plus a simple `set_text_streamer(callback)` hook if all you want is "print text as it arrives."
...
- [Backoff and jitter](/principles/backoff-and-jitter/) — the math behind the retry ladder described above.
- [Rate limiting](/principles/rate-limiting/) — the token bucket and `Retry-After` ladder, from scratch.
- [API client](/core-data/api-client/) — the trait producing these events.
- [The LLM turn](/engine/llm-turn/) — how the engine drives streaming vs non-streaming turns.
