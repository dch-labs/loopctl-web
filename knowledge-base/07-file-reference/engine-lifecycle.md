...
- `RunConfig` (`#[non_exhaustive]`): `max_turns: 200`, `parallel_tool_dispatch` (Sequential/8), `reset_managers: false`, `memory_top_k: 3`, `memory_include_provider_derived: false` (0.3.2 — entries tagged `provider-derived` stay out of injection unless opted in; builder `with_memory_include_provider_derived`); builders `with_max_turns`, `with_parallel_dispatch`.
...
Deep dives: [driver](../02-engine/02-driver-loop.md) · [config](../01-core-data/06-session-config.md).
