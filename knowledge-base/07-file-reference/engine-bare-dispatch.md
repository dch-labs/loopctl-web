# `src/engine/bare/dispatch.rs` — tool dispatch
...
- `dispatch_tool` — pipeline first if configured, else registry; panic-isolated (`catch_unwind`); unknown tool → soft error listing available names. The pipeline core (`middleware/tool_call.rs`) checks the cancel signal *before* invoking the tool (0.3.2), so a pre-cancelled call never starts — deterministically, not by `select!` branch luck.
...
Deep dive: [Tool dispatch](../02-engine/04-tool-dispatch.md).
