# Waves — running tool calls in parallel without stepping on each other
...
**Cancellation** is checked at every wave boundary, and each call races the cancel signal individually — see [cooperative cancellation](08-cooperative-cancellation.md).
...
Observers see the same event stream either mode (PREs batch, POSTs arrive by completion — pair by `tool_call_id`, not arrival order), and both modes use the *same* per-call pipeline, so every safety layer from [tool dispatch](../02-engine/04-tool-dispatch.md) fires identically.
...
- [Tool dispatch](../02-engine/04-tool-dispatch.md) — where waves sit in the per-call pipeline.
- [Tools](../01-core-data/02-tools.md) — declaring `is_safe_for_concurrent_execution` and `resource_key`.
- [Caching](09-caching-and-invalidation.md) — the epoch guard exists precisely because waves make reads and writes overlap.
