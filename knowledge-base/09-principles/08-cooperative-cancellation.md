# Cooperative cancellation — asking nicely, checking often
...
- **A cancelled turn is not recorded as a model failure.** When the error path sees `Cancelled`, it short-circuits *before* failure bookkeeping — the [fallback breaker](../03-safety/04-fallback.md) is not fed, no stream-failure event fires. A user changing their mind must not trip a circuit breaker; that's what actual errors are for.
...
The same contract applies through [MCP](../06-integration/02-mcp.md) — the server adapter races each remote call against the token and drops in-flight calls on cancel.

And cancellation can come from *inside*, too: anything holding the `Arc<CancelSignal>` — an [observer](../04-extensions/01-observers.md) deciding "this is going nowhere," a [hook](../04-extensions/02-hooks.md) enforcing a policy — can fire it, and the next checkpoint (almost always milliseconds away) picks it up.
...
- [Cancellation](../02-engine/05-cancellation.md) — the full mechanism page (API, wiring, tests).
- [Termination](../02-engine/07-termination.md) — where `Cancelled` sits among the other endings.
- [Backoff and jitter](05-backoff-and-jitter.md) — the other place "time" enters the engine's decisions.
