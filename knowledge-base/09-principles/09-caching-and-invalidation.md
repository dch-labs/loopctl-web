# Caching — remembering answers, forgetting them on time
...
3. the **original call's duration is reported**, not ~0 — so [health statistics](../03-safety/06-tool-health.md) keep measuring the real tool, never discovering an impossibly fast one;
...
- **A hit replays the *original* duration.** If the first read took 900 ms, the hit reports 900 ms, not 0 — so [health statistics](../03-safety/06-tool-health.md) keep tracking real latency instead of discovering an impossible fast tool.
...
- [Middleware](../03-safety/01-middleware.md) — the memoizer's configuration and pipeline position (innermost, beside the core).
- [Waves](11-scheduling-parallel-work.md) — the parallel execution that creates the epoch race in the first place.
- [Windows, averages, and similarity](07-measuring-repetition.md) — the same fingerprint-hashing idea used for repetition instead of caching.
