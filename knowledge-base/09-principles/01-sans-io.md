...
loopctl's engine is built this way. If you read one page about *how* loopctl works, the [big idea](../00-start-here/02-the-big-idea.md) page shows the split. This page explains the pattern itself: where it came from, what it buys, what it costs, and how to spot it in the code.
...
- **More explicit plumbing.** An ordinary loop can inline "call the model, use the reply." A sans-IO loop must phrase that as a step, a state, and a feed. The machinery in [the state machine](../02-engine/01-state-machine.md) and [the driver loop](../02-engine/02-driver-loop.md) pages *is* that plumbing.
...
- [The big idea](../00-start-here/02-the-big-idea.md) — the same split, as an introduction.
- [State machines](02-state-machines.md) — the shape the pure core takes.
- [The state machine](../02-engine/01-state-machine.md) · [the driver loop](../02-engine/02-driver-loop.md) — the two halves in full detail.
