# Backoff and jitter — the math of waiting before you retry
...
|---|---|---|---|---|
| **Stream transport** | [stream handler](../01-core-data/04-stream-events.md) | 100 ms → 10 s, ±10% jitter | 4 (1 + 3 retries) | config, automatic |
| **Rate limit** | stream handler | server's `Retry-After` (or 5 s) → 60 s | 5, then escalate | config, automatic |
| **Tool recovery** | [recovery](../03-safety/05-reflection.md) | 100 ms → 30 s | ≤ 5 hard ceiling | your `RecoveryStrategy` |
| **Timeout retry** | [timeout middleware](../03-safety/01-middleware.md) | deadline **doubles** per retry | 1 + `max_retries` | middleware config |

Two structural choices in that table are worth calling out, because they're principles, not just settings:

**Rate limits get their own budget.** A 429 ("slow down") is not a transient glitch — it's the server *pricing* your usage. loopctl keeps the rate-limit ladder separate from the transport ladder so a rate-limit storm cannot eat the retries you need for genuine network failures (nor vice versa). When the rate-limit budget is spent, the failure **escalates** — it becomes a `RateLimitEscalation` that the [fallback breaker](../03-safety/04-fallback.md) understands, rather than a retry in disguise. And the ladder honors `Retry-After` (the server's own "wait this long" header) whenever it's present, capped at 60 s — the server knows its load better than any formula.
...
The escalation at count 4 is the interesting rung: it stops retrying *this model* and reports upward, where the [fallback system](../03-safety/04-fallback.md) can reroute the turn to another model entirely. Retrying harder against a service that is explicitly rationing you is never the answer — the ladder is built to hand the problem to a different mechanism at exactly the point where more patience stops being creditable.
...
- [Stream events](../01-core-data/04-stream-events.md) — where the transport and rate-limit ladders live.
- [Reflection & recovery](../03-safety/05-reflection.md) — the tool-side ladder.
- [Circuit breakers](06-circuit-breakers.md) — what happens when retries *keep* failing: stop retrying, start probing.
