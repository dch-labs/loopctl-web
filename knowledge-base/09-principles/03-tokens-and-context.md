# Tokens and context windows — the meter is always running
...
- **After compaction, the estimate is the manager's own re-count**, not the compactor's self-report — the number that decides "when next" is always produced by the same counter that decided "when now". Swap in a different counter at any layer and rows like the last two stop agreeing: compact to 3,800 by one ruler, measure 5,100 by another, compact again — the [flapping](../08-cookbook/01-gotchas.md) loop that gotcha #6 warns about, visible here as two rows that must share a ruler.
...
The brain never measures anything itself — the driver feeds it a number, and the comparison against the lines is the brain's entire job. (That's the [sans-IO](01-sans-io.md) discipline: the meter lives in the hands, the rule lives in the brain.)

### The one-unit rule

One consistency rule sits above all tuning: **every token decision must be made with the same counter.** The trigger asks "are we past 80%?" and the post-compaction check asks "did we get smaller?" — if those two questions use *different* units (a heuristic here, the provider's billed count there), the system can flap: compact, measure small, grow back, measure big, compact again, forever. Install one counter on the `ContextManager` and the driver reuses it everywhere. (This is [gotcha #6](../08-cookbook/01-gotchas.md) in the cookbook.)
...
And after a compaction pass, the [no-progress guard](../02-engine/06-compaction.md) compares before/after **with the same counter**: if nothing was shaved, the run ends with `ContextExceeded` rather than looping between "compact" and "call" forever.

---

## Related pages

- [Compaction](../02-engine/06-compaction.md) — what actually happens when the lines are crossed.
- [Session config](../01-core-data/06-session-config.md) — where the window and threshold live.
- [Sans-IO](01-sans-io.md) — why the brain stores a *number someone fed it*, never a measurement it took itself.
