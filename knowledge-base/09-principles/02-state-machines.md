# State machines — one situation at a time
...
loopctl's engine brain ([`LoopMachine`](../02-engine/01-state-machine.md)) is a state machine. This page explains why an agent loop is a natural fit for the pattern, and what the pattern buys once it's there.
...
- **Asking is not a transition.** `next_step()` — "what should happen next?" — is a pure question. It reads the state and answers `CallLLM`, `CallTools`, `Compact`, or `Done`. It never changes the state. All change flows through feeds. (This is the sans-IO discipline from the [previous page](01-sans-io.md) expressed as machine rules.)
...
|---|---|---|
| The engine brain | `Start / AwaitingModel / AwaitingTools / AwaitingCompaction / Terminal` | [the state machine](../02-engine/01-state-machine.md) |
| Model fallback breaker | `Primary / Fallback / Recovering` | [fallback](../03-safety/04-fallback.md) |
| Per-tool circuit breaker | `Closed / Open / HalfOpen` | [tool health](../03-safety/06-tool-health.md) |
| Stream assembly | open slots per lane (text / tool / thinking) | [stream events](../01-core-data/04-stream-events.md) |
...
- [Sans-IO](01-sans-io.md) — the pattern this one lives inside.
- [The state machine](../02-engine/01-state-machine.md) — every rule of the brain, in detail.
- [The driver loop](../02-engine/02-driver-loop.md) — the shell that drives these transitions.
- [Circuit breakers](06-circuit-breakers.md) — the pattern as applied to failing things.
