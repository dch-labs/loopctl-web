# Rate limiting — spending requests wisely
...
Prevention is a guess; the server's answer is a measurement. When a 429 (or a 503/529 "overloaded" — same treatment) still arrives, the [stream handler](../01-core-data/04-stream-events.md) switches to the reactive ladder:
...
3. **Escalate.** Still limited after 3 tries → the failure becomes `RateLimitEscalation` and is reported to the [model circuit breaker](../03-safety/04-fallback.md), which can reroute the whole turn to a backup model. Deliberately, this does **not** silently retry the same model unstreamed — a rate limit is charged against the model's quota no matter how the request travels.
...
> One accounting note: rate-limit retries have a **separate budget** from transport retries (network blips, 5xx). A storm of 429s cannot consume the patience you need for genuine network failures — see [backoff and jitter](05-backoff-and-jitter.md) for the other budget.

---

## Related pages

- [Stream events](../01-core-data/04-stream-events.md) — where both layers are wired.
- [Backoff and jitter](05-backoff-and-jitter.md) — the retry math the reactive ladder uses.
- [Circuit breakers](06-circuit-breakers.md) — where rate-limit escalation lands.
