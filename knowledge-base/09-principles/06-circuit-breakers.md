# Circuit breakers — failing fast, recovering carefully
...
| | Model breaker ([fallback](../03-safety/04-fallback.md)) | Tool breaker ([tool health](../03-safety/06-tool-health.md)) |
|---|---|---|
...
Both breakers report their refusals as **soft errors** the model can read: *"tool temporarily unavailable: circuit breaker open"*. The run continues; the model routes around the sick tool or waits. A breaker that crashed your agent to protect it would be trading one failure for a worse one — see [soft and hard errors](04-soft-and-hard-errors.md) for the principle. (For tools, refusal isn't even the only policy: the `HealthRouter` can substitute a healthy backup tool instead.)
...
Retries are optimism at millisecond scale; breakers are pessimism at minute scale. You need both, in that order — see the [previous page](05-backoff-and-jitter.md) for the optimism half.

---

## Related pages

- [Fallback](../03-safety/04-fallback.md) · [tool health](../03-safety/06-tool-health.md) — the two real breakers, in full detail.
- [Backoff and jitter](05-backoff-and-jitter.md) — what happens before the breaker gets involved.
- [State machines](02-state-machines.md) — the pattern both breakers are instances of.
