# Text matching — similarity without understanding
...
The answer is a family of **character-and-word tricks**: similarity measures that understand nothing about meaning but are fast, dependency-free, and wrong in known, bounded ways. loopctl uses three of them (plus Jaccard, covered with [convergence detection](07-measuring-repetition.md)). This page explains each with its real formula and a worked example.
...
The built-in memory store ([memory](../04-extensions/03-memory.md)) must rank which stored entries a new question should surface. No embeddings — just counting shared words. Each candidate entry scores:
...
Two honest limits in the built-in store, deliberate: matching is transparent word overlap — never semantic — and popularity and forgetting live outside the scorer. A retrieval that actually matched records an access stamp, and the consolidation pass (0.3.2: decay, merge, prune) folds those stamps into `access_count` and decays relevance by category — so being retrieved rewards an entry, but the *matching itself* stays word counting, small enough to verify by reading.
...
- [Memory](../04-extensions/03-memory.md) — the store the word-overlap scorer serves.
- [Middleware](../03-safety/01-middleware.md) — where the suggestion layer sits.
- [Windows, averages, and similarity](07-measuring-repetition.md) — Jaccard's full story.
- [Structured output](../06-integration/01-structured-output.md) — the strict paths around the brace scanner.
