# Model switch — changing the model mid-session, atomically
...
This is the *manual*, host-driven switch. The *automatic* one (on provider failures) is the [fallback manager](../03-safety/04-fallback.md) — different mechanism, same notification.
...
- [Fallback](../03-safety/04-fallback.md) — the automatic model switching.
- [Observers](../04-extensions/01-observers.md) — `on_model_switched` and friends.
