# Cancellation — stopping a run cleanly, mid-anything
...
You don't need a separate task. Anything holding the `Arc<CancelSignal>` can fire — including code inside an [observer](../04-extensions/01-observers.md) or a [hook](../04-extensions/02-hooks.md) running on the engine's tasks. The next checkpoint (almost always within milliseconds) picks it up.
...
- [Cooperative cancellation](../09-principles/08-cooperative-cancellation.md) — the pattern behind this design: why a shared flag beats a kill switch.
- [Termination](07-termination.md) — how Cancelled compares to the other endings.
- [The driver loop](02-driver-loop.md) — the error path a cancel travels.
- [File reference: cancel.rs](../07-file-reference/cancel.md)
