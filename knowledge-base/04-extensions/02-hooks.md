# Hooks — allow, block, or steer before things happen [feature: hooks]

[Observers](01-observers.md) watch. **Hooks decide.** A hook can block a tool call before it runs, veto a compaction pass, or steer the summarizer's instructions. Enable with `features = ["hooks"]`. Sources: `src/hooks/`, built-ins in `src/hooks/builtin/`.
...
| **[Middleware](../03-safety/01-middleware.md)** | yes (pipeline-level) | input *and* output | around the tool call | per-call policy, rewriting, caching |
...
- A pre-compact veto at over-threshold is a decision to **stop** (the no-progress guard ends the run), not a decision to proceed — see [compaction](../02-engine/06-compaction.md).
...
- [Observers](01-observers.md) — the watching counterpart.
- [Permission](../03-safety/08-permission.md) — the pipeline-level gate.
