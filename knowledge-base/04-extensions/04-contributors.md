# Contributors — a message before every turn
...
- Extras count toward the context estimate the compaction trigger uses (see [the LLM turn](../02-engine/03-llm-turn.md)).

## Contributors vs memory — when to use which

Both inject per-turn context. The difference is *where the content comes from*:

| | Contributor | [Memory](03-memory.md) |
|---|---|---|
...
- [Memory](03-memory.md) — the retrieved-context counterpart.
- [Presets](../06-integration/04-presets.md) — `GoalReminder` in the Constrained profile.
