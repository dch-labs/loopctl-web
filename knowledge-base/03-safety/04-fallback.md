# Fallback — when the model itself is the problem
...
- **Rate-limit escalations are special.** When the [stream handler](../01-core-data/04-stream-events.md) exhausts its retry budget on 429/503/529, the error arrives as `RateLimitEscalation` and counts as `RateLimit` — during recovery, that re-trips immediately ("the primary is still refusing load").
...
| | [`switch_model`](../02-engine/08-model-switch.md) | fallback manager |
|---|---|---|
...
- [Circuit breakers](../09-principles/06-circuit-breakers.md) — the pattern this manager implements, from scratch.
- [Stream handler](../01-core-data/04-stream-events.md) — the retry ladder that feeds this breaker.
- [Termination](../02-engine/07-termination.md) — `FallbackExhausted` in context.
