# Providers — talking to real models
...
- **Hot model swap** — `set_model` works on all shipped clients, enabling [`switch_model`](../02-engine/08-model-switch.md).
...
- [OpenAI & the compatible family](02-openai.md) — OpenAiClient, Ollama, DeepSeek, Grok, Azure, Moonshot, self-hosted.
- [Anthropic](03-anthropic.md) — Claude, thinking blocks, forced-tool JSON.
- [Gemini](04-gemini.md) — Gemini quirks: model-in-URL, thoughts.
- [Bedrock](05-bedrock.md) — AWS signing, event-stream decoding, Converse.
- [SSE under the hood](06-sse.md) — how streaming bytes become events.
- [Grammar constraints](07-grammar.md) — vLLM `guided_json` for local models.

## Picking for local development

`provider::ollama("qwen3")` against a local Ollama server is the cheapest way to develop against a *real* model — no cloud key, no cost, and full tool-calling. The `testing` feature's `MockApiClient` covers the no-model-at-all tier. See [testing](../06-integration/05-testing.md).
